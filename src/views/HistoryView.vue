...
const loadHistoryArticles = async (silent = false) => {
  if (!silent) loading.value = true
  const results = []
  for (const id of userStore.historyIds) {
    try {
      const article = await getArticle(id)
      if (article) results.push(article)
    } catch { /* 跳过已删除的文章 */ }
  }
  historyArticles.value = results.sort((a, b) => new Date(b.date) - new Date(a.date))
  loading.value = false
...
const onPullEnd = async () => {
  ...
  if (pullDistance.value >= PULL_THRESHOLD) {
    ...
    await loadHistoryArticles(true)
  }
  pullDistance.value = 0
}

onMounted(() => { loadHistoryArticles() })

const goBack = () => router.back()
const goToDetail = (id) => router.push(`/article/${id}`)

/** 清空浏览历史 — API 优先，本地 fallback */
...
<template>
  <main class="history-page" aria-label="浏览历史">
    ...
    <!-- 文章列表 -->
    <div
      v-else-if="historyArticles.length > 0"
      class="article-list"
